...
|----------|-------------|-------------|
| Первый запуск | `Unknown` → `Healthy` | Probe успешен с первой попытки |
| Сбой сервиса | `Healthy` → `Unhealthy` | Probe вернул ошибку, `consecutive_failures = 1` |
| Рестарт | `Unhealthy` → `Recovering` | Policy решила рестартовать, `attempt = 1` |
| Исчерпаны попытки | `Recovering` → `Failed` | `attempt > severity.max()` |
| Зависимость упала | `Healthy` → `BlockedByDep` | Root cause — другой сервис |
| Breaker сработал | `Unhealthy` → `Suppressed` | `restarts_in_window >= threshold` |
...
> 💡 **Что такое «Phase 4»?**  
> Это команды, для которых уже написаны типы данных и логика в `reduce`/`policy`, но ещё не реализовано выполнение в `effect/`.  
> Они принимаются демоном, но не выполняют реальных действий (no-op).  
> 
> **План:** реализовать `effect/` для:
> - [ ] `RunDiskCleanup` — очистка диска через `apt clean`, `journalctl --vacuum`
> - [ ] `CheckAptUpdates` / `CheckDockerUpdates` — проверка обновлений
> - [ ] `WriteIncident` — запись инцидентов в файл/системный лог
> - [ ] `TriggerPicoClaw` — интеграция с AI-мостом
